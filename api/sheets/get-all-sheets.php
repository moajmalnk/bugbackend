<?php
/**
 * Get All Sheets Endpoint (Admin Only)
 * GET /api/sheets/get-all-sheets
 * Returns all sheets from all users (admins, developers, testers, and others) grouped by project (admin only)
 */
...
try {
    ...
    error_log("Listing all sheets for admin user: {$userId}");
    
    // Get sheets
    $result = $controller->listAllSheets($userId, $includeArchived);
    
    http_response_code(200);
    echo json_encode($result);
    
} catch (Exception $e) {
    error_log("Error in get-all-sheets.php: " . $e->getMessage());
