<?php
/**
 * Send Magic Link Debug Version
 * This version includes extensive error logging to identify the issue
 */
...
// Log the request
error_log("Magic Link Debug: Request started");

try {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        error_log("Magic Link Debug: Method not allowed - " . $_SERVER['REQUEST_METHOD']);
        http_response_code(405);
        echo json_encode(['success' => false, 'message' => 'Method not allowed']);
        exit();
    }
    
    error_log("Magic Link Debug: Processing POST request");
    
    $input = json_decode(file_get_contents('php://input'), true);
    error_log("Magic Link Debug: Input received - " . json_encode($input));
    
    if (!isset($input['email']) || empty($input['email'])) {
        error_log("Magic Link Debug: Email not provided");
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        error_log("Magic Link Debug: Invalid email format - " . $email);
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Invalid email format']);
        exit();
    }
    
    error_log("Magic Link Debug: Email validated - " . $email);
    ...
    $db = getDBConnection();
    if (!$db) {
        error_log("Magic Link Debug: Database connection failed");
        throw new Exception("Database connection failed");
    }
    
    error_log("Magic Link Debug: Database connected successfully");
    ...
    $user = $result;
    error_log("Magic Link Debug: User found - " . json_encode($user));
    
    // Generate magic link token
    $token = bin2hex(random_bytes(32));
    $expires_at = date('Y-m-d H:i:s', strtotime('+15 minutes'));
    error_log("Magic Link Debug: Token generated - " . $token);
    
    // Check if magic_links table exists
    $table_check = $db->query("SHOW TABLES LIKE 'magic_links'");
    if (!$table_check || $table_check->rowCount() === 0) {
        error_log("Magic Link Debug: magic_links table does not exist");
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Magic links table not found. Please run the migration script.']);
        exit();
    }
    
    error_log("Magic Link Debug: magic_links table exists");
    
    // Store magic link in database
    error_log("Magic Link Debug: Deleting existing magic links for user - " . $user['id']);
    $delete_stmt = $db->prepare("DELETE FROM magic_links WHERE user_id = ?");
    if (!$delete_stmt) {
        error_log("Magic Link Debug: Failed to prepare delete statement");
    ...
    error_log("Magic Link Debug: Inserting new magic link");
    $stmt = $db->prepare("
        INSERT INTO magic_links (user_id, token, email, expires_at, created_at) 
        VALUES (?, ?, ?, ?, NOW())
    ");
    if (!$stmt) {
        error_log("Magic Link Debug: Failed to prepare insert statement");
    ...
    error_log("Magic Link Debug: Magic link stored successfully");
    ...
    // Test email sending
    error_log("Magic Link Debug: Attempting to send email");
    $email_sent = sendMagicLinkEmail($user['email'], $user['username'], $magic_link);
    
    if (!$email_sent) {
        error_log("Magic Link Debug: Email sending failed");
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to send magic link email']);
        exit();
    }
    
    error_log("Magic Link Debug: Email sent successfully");
    ...
    error_log("Magic Link Debug: Exception caught - " . $e->getMessage());
    error_log("Magic Link Debug: Stack trace - " . $e->getTraceAsString());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Internal server error: ' . $e->getMessage()]);
}

function sendMagicLinkEmail($email, $username, $magic_link) {
    error_log("Magic Link Debug: sendMagicLinkEmail called for - " . $email);
...
© 2025 BugRicer. All rights reserved.
        ...
        $result = sendEmail($email, $subject, $html_body, $text_body);
        error_log("Magic Link Debug: sendEmail result - " . ($result ? 'true' : 'false'));
        return $result;
        
    } catch (Exception $e) {
        error_log("Magic Link Debug: Email sending exception - " . $e->getMessage());
        return false;
    }
}
?>
