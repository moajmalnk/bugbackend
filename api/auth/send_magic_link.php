 ...
 * POST /api/auth/send_magic_link.php
 */
...
class MagicLinkAPI extends BaseAPI {
    private $db;
    ...
    public function handleRequest() {
        ...
        try {
            ...
            $delete_stmt = $this->db->prepare("DELETE FROM magic_links WHERE user_id = ?");
            ...
            $magic_link = $this->generateMagicLink($token);
            
            // Send magic link email
            $email_sent = $this->sendMagicLinkEmail($user['email'], $user['username'], $magic_link);
            
            if (!$email_sent) {
                $this->sendResponse(['success' => false, 'message' => 'Failed to send magic link email'], 500);
                return;
            }
    ...
    private function generateMagicLink($token) {
        $base_url = $this->getBaseUrl();
        return $base_url . "/login?magic_token=" . $token;
    }
