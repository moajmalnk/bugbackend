<?php
// Fix users table collation to match password_resets table
...
try {
    ...
    echo "=== Fixing Users Table Collation ===\n";
    ...
    // Fix the collation mismatch by changing users table to match password_resets
    echo "\n--- Fixing users table collation ---\n";
    
    // Step 1: Change users table collation to utf8mb4_unicode_ci
    echo "Step 1: Changing users table collation...\n";
    try {
        $pdo->exec("ALTER TABLE users CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        echo "✓ Users table collation changed to utf8mb4_unicode_ci\n";
    } catch (Exception $e) {
        echo "Error changing users table collation: " . $e->getMessage() . "\n";
    }
    
    // Step 2: Specifically fix the id column collation
    echo "Step 2: Fixing users.id column collation...\n";
    try {
        $pdo->exec("ALTER TABLE users MODIFY COLUMN id VARCHAR(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL");
        echo "✓ users.id column collation fixed\n";
    } catch (Exception $e) {
        echo "Error fixing users.id collation: " . $e->getMessage() . "\n";
    }
    
    // Step 3: Fix other string columns in users table
    echo "Step 3: Fixing other string columns in users table...\n";
    try {
        $pdo->exec("ALTER TABLE users MODIFY COLUMN username VARCHAR(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL");
        $pdo->exec("ALTER TABLE users MODIFY COLUMN email VARCHAR(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL");
        $pdo->exec("ALTER TABLE users MODIFY COLUMN phone VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        $pdo->exec("ALTER TABLE users MODIFY COLUMN password VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL");
        $pdo->exec("ALTER TABLE users MODIFY COLUMN fcm_token VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        echo "✓ Other string columns collation fixed\n";
    } catch (Exception $e) {
        echo "Error fixing other columns: " . $e->getMessage() . "\n";
    }
    ...
    echo "Users table collation: " . $new_users_collation . "\n";
    
    $stmt = $pdo->query("SELECT COLUMN_NAME, COLLATION_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'id'");
    $new_users_id_collation = $stmt->fetch(PDO::FETCH_ASSOC)['COLLATION_NAME'];
    echo "Users.id collation: " . $new_users_id_collation . "\n";
    
    $stmt = $pdo->query("SELECT TABLE_COLLATION FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'password_resets'");
    $new_password_resets_collation = $stmt->fetch(PDO::FETCH_ASSOC)['TABLE_COLLATION'];
    echo "Password_resets table collation: " . $new_password_resets_collation . "\n";
    
    $stmt = $pdo->query("SELECT COLUMN_NAME, COLLATION_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'password_resets' AND COLUMN_NAME = 'user_id'");
    $new_password_resets_user_id_collation = $stmt->fetch(PDO::FETCH_ASSOC)['COLLATION_NAME'];
    echo "Password_resets.user_id collation: " . $new_password_resets_user_id_collation . "\n";
